...
8. 参数解析顺序：已有 `expression`（及 `valueKind`）→ `_raw/field-memory.json`（或同文档已确认事件）同「key + 文档说明」上次确认值 → `hint.api` → hint 图/note + 示意图 → 仓级惯例 → 同文件已有埋点 → 按「用途说明」追变量。扫描到的候选全部写入 `candidates[]`，禁止丢弃。同档能排出唯一最佳候选时写入 `expression` / `preferredCandidateId`；同档完全打平则 `expression` 可空，但 `candidates[]` 必须保留，交给确认页选择。只有源码里没有可用候选时才把 `expression` 留空。`confidence` 由程序推导；`medium` / `low` **不得清空**已有 `expression`。记忆回填规则见 `reference/confirm-flow.md`：key + 文档说明都一致时，可覆盖 `low` / `medium` AI 候选（原候选记入 `memoryReplaced`），但不覆盖 `high` 与已人工确认的参数；描述不一致或缺说明则只回填空表达式。记忆回填后 `confidence` 为 `medium`，**仍须人工确认**。代码里已有调用优先于记忆。不回填 `targetFile` / `uicode`。用户在落库页可 **确认 / 修改 / 从候选中选择**，或在无候选时填 **JS 表达式** / **备注**（`valueKind: prompt`）。备注非空也算已给出取值。
