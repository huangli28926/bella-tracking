...
const {
  ...
  describeSteps,
  filterChainByEvt
} = require('../chain/accept-chain')
const { buildAcceptReport, collectEmptyParams, enrichReportMedia, renderAcceptReport } = require('../report/accept-report')
const { lockChainExpect, lockImplPayload } = require('../../lib/lock-doc-uicode')
const { runtimeKeepLockedPath } = require('../chain/resolve-accept-path')
const {
  defaultStoragePath,
  ensureLoggedIn,
  isLoginUrl,
...
const { assertValidImpl } = require('../impl/validate-impl')
...
function loadOrBuildChain(paths, args, urls) {
  ...
  if (needRebuild) {
    ...
    if (args['write-impl']) {
      const { patchImplAccept } = require('../chain/build-accept-chain')
...
async function main() {
  ...
  let results
  if (planOnly) {
    results = require('../report/accept-report').buildPlanResults(chain)
  } else {
  ...
  if (!planOnly) {
    try {
      const { defaultFinalPaths, renderFinalToFile } = require('../report/final-report')
